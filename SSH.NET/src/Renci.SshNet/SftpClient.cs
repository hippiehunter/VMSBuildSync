    ...
    public class SftpClient : BaseClient, ISftpClient
    {
        ...
        private IEnumerable<FileInfo> InternalSynchronizeDirectories(string sourcePath, string destinationPath, string searchPattern, SftpSynchronizeDirectoriesAsyncResult asynchResult)
        {
            ...
            foreach (var localFile in sourceFiles)
            {
                ...
                var localFullName = localFile.FullName;
                if (string.IsNullOrWhiteSpace(localFile.Extension))
                {
                    localFileName = localFile.Name + "."; //VMS is weird if you dont have a file extension
                }
                var isDifferent = !destDict.ContainsKey(localFileName);

                if (!isDifferent)
                {
                    var temp = destDict[localFileName];
                ...
                if (isDifferent)
                {
                    var remoteFileName = string.Format(CultureInfo.InvariantCulture, @"{0}/{1}", destinationPath, localFileName);
                    try
                    {
        ...
        {
            return FilterVersions(InternalListDirectory1(path, listCallback));
        }
        private IEnumerable<SftpFile> InternalListDirectory1(string path, Action<int> listCallback)
        {
            if (path == null)
                throw new ArgumentNullException("path");
        ...
        private IEnumerable<SftpFile> FilterVersions(IEnumerable<SftpFile> files)
        {
            var destDict = new Dictionary<string, SftpFile>();
